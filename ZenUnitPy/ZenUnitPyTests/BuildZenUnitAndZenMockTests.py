...
"""Usage: python3 BuildZenUnitAndZenMock.py --cmake-generator=<CMakeGenerator> --cmake-architecture=<x64OrWin32> --cmake-build-type=<CMakeBuildType> --cmake-definitions=<QuotedSpaceSeparatedCMakeDefinitions>""", 1)
   ...
   def main_ArgsLengthIs5_CMakes_Builds_Installs_test(self):
      ...
      def testcase(platformSystem, expectLinux, _1, _2, _3, _4, _5, _6):
         with self.subTest(f'{platformSystem}, {expectLinux}'):
            ...
            if expectLinux:
               BuildZenUnitAndZenMock.linux_cmake_build_install.assert_called_once_with(self.cmakeGenerator, self.cmakeArchitecture, self.cmakeBuildType, self.cmakeDefinitions)
               ...
               BuildZenUnitAndZenMock.windows_cmake_build_install.assert_called_once_with(self.cmakeGenerator, self.cmakeArchitecture, self.cmakeBuildType, self.cmakeDefinitions)
   ...
   def linux_cmake_and_build_CMakes_BuildsWithNinja_RunsCMakeInstall_test(self):
      ...
      def testcase(cmakeDefinitions, expectedCMakeDefinitionsArgument, _1, _2, _3):
         with self.subTest(f'{cmakeDefinitions, expectedCMakeDefinitionsArgument}'):
            #
            BuildZenUnitAndZenMock.linux_cmake_build_install(self.cmakeGenerator, self.cmakeArchitecture, self.cmakeBuildType, cmakeDefinitions)
            #
   ...
   def windows_cmake_build_install_CMakes_BuildsWithMSBuild_RunsCMakeInstall_test(self):
      ...
      def testcase(cmakeDefinitions, expectedCMakeDefinitionsArgument, _1, _2, _3):
         with self.subTest(f'{cmakeDefinitions, expectedCMakeDefinitionsArgument, }'):
            #
            BuildZenUnitAndZenMock.windows_cmake_build_install(self.cmakeGenerator, self.cmakeArchitecture, self.cmakeBuildType, cmakeDefinitions)
            #
