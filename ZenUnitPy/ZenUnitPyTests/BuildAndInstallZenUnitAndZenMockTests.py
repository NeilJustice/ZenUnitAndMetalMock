...
from ZenUnitPy import ArgParser, CMake, BuildAndInstallZenUnitAndZenMock, Process, UnitTester, Util
import Random
   ...
   def main_ArgsLengthIsNot5_PrintsUsageAndExits1_test(self):
      ...
      def testcase(argsLength, _1):
         with self.subTest(f'{argsLength}'):
            ...
            #
            BuildAndInstallZenUnitAndZenMock.main(invalidArgs)
            #
...
"""Usage: python BuildAndInstallZenUnitAndZenMock.py --cmake-generator=<CMakeGenerator> --cmake-architecture=<x64OrWin32> --cmake-build-type=<CMakeBuildType> --cmake-definitions=<QuotedSpaceSeparatedCMakeDefinitions>""", 1)
   ...
   def main_ArgsLengthIs5_CMakes_Builds_Installs_test(self):
      ...
      @patch('ZenUnitPy.BuildAndInstallZenUnitAndZenMock.linux_cmake_build_install', spec_set=True)
      @patch('ZenUnitPy.BuildAndInstallZenUnitAndZenMock.windows_cmake_build_install', spec_set=True)
      ...
      def testcase(platformSystem, expectLinux, _1, _2, _3, _4, _5, _6):
         with self.subTest(f'{platformSystem}, {expectLinux}'):
            ...
            #
            BuildAndInstallZenUnitAndZenMock.main(args)
            #
            ...
            if expectLinux:
               BuildAndInstallZenUnitAndZenMock.linux_cmake_build_install.assert_called_once_with(self.cmakeGenerator, self.cmakeArchitecture, self.cmakeBuildType, self.cmakeDefinitions)
               ...
               BuildAndInstallZenUnitAndZenMock.windows_cmake_build_install.assert_called_once_with(self.cmakeGenerator, self.cmakeArchitecture, self.cmakeBuildType, self.cmakeDefinitions)
   ...
   def linux_cmake_and_build_CMakes_BuildsWithNinja_RunsCMakeInstall_test(self):
      ...
      def testcase(cmakeDefinitions, expectedCMakeDefinitionsArgument, _1, _2, _3):
         with self.subTest(f'{cmakeDefinitions, expectedCMakeDefinitionsArgument}'):
            #
            BuildAndInstallZenUnitAndZenMock.linux_cmake_build_install(self.cmakeGenerator, self.cmakeArchitecture, self.cmakeBuildType, cmakeDefinitions)
            #
   ...
   def windows_cmake_build_install_CMakes_BuildsWithMSBuild_RunsCMakeInstall_test(self):
      ...
      def testcase(cmakeDefinitions, expectedCMakeDefinitionsArgument, _1, _2, _3):
         with self.subTest(f'{cmakeDefinitions, expectedCMakeDefinitionsArgument, }'):
            #
            BuildAndInstallZenUnitAndZenMock.windows_cmake_build_install(self.cmakeGenerator, self.cmakeArchitecture, self.cmakeBuildType, cmakeDefinitions)
            #
