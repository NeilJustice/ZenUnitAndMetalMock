...
namespace ZenUnit
{
   ...
   TEST(TwoArgAny_1ElementCollection_ElementDoesNotMatchPredicate_ReturnsFalse)
   {
      ...
      VECTORS_ARE_EQUAL(expectedPredicateArgs, s_predicateArgs);
      IS_FALSE(anyElementMatchesPredicate);
   }
   ...
   TEST(TwoArgAny_2ElementCollection_FirstOfTwoElementsMatchesPredicate_CallsPredicateOnce_ReturnsTrue)
   {
      ...
      IS_FALSE(ElementValueOneReturnsTruePredicate(T(2), Arg2Type{}));
   }
   ...
   {
      THROWS_EXCEPTION(PredicateThatThrowsIfCalled(T{}, Arg2Type{}), invalid_argument, "unexpected call");
   }
...
template<
   template<typename...>
   typename CollectionType, typename T, typename Arg2Type>
vector<pair<T, Arg2Type>> ZenUnit::TwoArgAnyerTests<CollectionType, T, Arg2Type>::s_predicateArgs;
