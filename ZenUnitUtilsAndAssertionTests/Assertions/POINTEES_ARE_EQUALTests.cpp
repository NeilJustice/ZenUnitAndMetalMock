...
" Because: ARE_EQUAL(*expectedPointer, *actualPointer) failed",
"Expected: \"" + expectedPointee + "\"",
"  Actual: \"" + actualPointee + "\"",
"File.cpp(1)",
"File.cpp(1)"));
...
"File.cpp(1)"));
   ...
   {
      const unique_ptr<const UserType> expectedPointer = make_unique<UserType>(1);
      const unique_ptr<const UserType> actualPointer = make_unique<UserType>(2);
      THROWS_EXCEPTION(POINTEES_ARE_EQUAL(expectedPointer, actualPointer),
         Anomaly, TestUtil::NewlineConcat("",
"  Failed: POINTEES_ARE_EQUAL(expectedPointer, actualPointer)",
"Expected: UserType@1",
...
"File.cpp(1)"));
   ...
   TEST(ExpectedPointerIsNotNull_ActualPointerIsNotNull_PointeesAreEqual_DoesNotThrowExceptionExceeption__UserTypePointersTestCase)
   {
      ...
      const shared_ptr<const UserType> expectedPointer = make_shared<UserType>(randomInt);
      const shared_ptr<const UserType> actualPointer = make_shared<UserType>(randomInt);
      POINTEES_ARE_EQUAL(expectedPointer, actualPointer);
   }
