...
|`ZenUnit::RandomFloatBetween(float inclusiveMinValue, float inclusiveMaxValue)`|Returns a random `float` between `inclusiveMinValue` and `inclusiveMaxValue` selected from a `std::uniform_real_distribution<float>` according to following probabilities:<br><br>1/6 probability: `std::numeric_limits<float>::lowest()`<br>1/6 probability: `0.0f`<br>1/6 probability: `std::numeric_limits<float>::max()`<br>3/6 probability: `float` value between `inclusiveMinValue` and `inclusiveMaxValue`|
|`ZenUnit::RandomDoubleBetween(double inclusiveMinValue, double inclusiveMaxValue)`|Returns a random `double` between `inclusiveMinValue` and `inclusiveMaxValue` selected from a `std::uniform_real_distribution<double>` according to following probabilities:<br><br>1/6 probability: `std::numeric_limits<double>::lowest()`<br>1/6 probability: `0.0`<br>1/6 probability: `std::numeric_limits<double>::max()`<br>3/6 probability: `double` value between `inclusiveMinValue` and `inclusiveMaxValue`|
