...
namespace ZenMock
{
   TESTS(FourArgumentZenMockerTests)
   ...
   using ZenMockerType = FourArgumentZenMocker<int, int, int, int, ExceptionThrowerMock>;
   ...
   {
      IS_FALSE(_zenMocker->_wasExpected);
      THROWS_EXCEPTION(_zenMocker->ZenMockIt(1, 2, 3, 4), UnexpectedCallException,
         UnexpectedCallException::MakeWhat(_zenMockedFunctionSignature, 1, 2, 3, 4));
   }
   ...
   TEST(ZenMockIt_ExpectedTrue_IncrementsNumberOfCalls_CallsZenMockThrowIfExceptionSet)
   {
      ...
      DOES_NOT_THROW(_zenMocker->CalledOnceWith(1, 2, 3, 4));
      SetAssertedTrueToNotFailDueToExpectedButNotAsserted();
   }

   RUN_TESTS(FourArgumentZenMockerTests)
}
