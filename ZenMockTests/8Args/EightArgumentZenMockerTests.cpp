...
namespace ZenMock
{
   TESTS(EightArgumentZenMockerTests)
   ...
   using ZenMockerType = EightArgumentZenMocker<int, int, int, int, int, int, int, int, ExceptionThrowerMock>;
   ...
   {
      IS_FALSE(_zenMocker->_wasExpected);
      THROWS_EXCEPTION(_zenMocker->ZenMockIt(1, 2, 3, 4, 5, 6, 7, 8), UnexpectedCallException,
         UnexpectedCallException::MakeWhat(_zenMockedFunctionSignature, 1, 2, 3, 4, 5, 6, 7, 8));
   }
   ...
   TEST(ZenMockIt_ExpectedTrue_IncrementsNumberOfCalls_CallsZenMockThrowIfExceptionSet)
   {
      ...
      //
      _zenMocker->ZenMockIt(1, 2, 3, 4, 5, 6, 7, 8);
      //
      ...
      DOES_NOT_THROW(_zenMocker->CalledOnceWith(1, 2, 3, 4, 5, 6, 7, 8));
      SetAssertedTrueToNotFailDueToExpectedButNotAsserted();
   }

   RUN_TESTS(EightArgumentZenMockerTests)
}
