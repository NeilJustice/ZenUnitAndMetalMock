...
namespace ZenMock
{
   TESTS(SevenArgumentZenMockerTests)
   ...
   using ZenMockerType = SevenArgumentZenMocker<int, int, int, int, int, int, int, ExceptionThrowerMock>;
   ...
   {
      IS_FALSE(_zenMocker->_wasExpected);
      THROWS_EXCEPTION(_zenMocker->ZenMockIt(1, 2, 3, 4, 5, 6, 7), UnexpectedCallException,
         UnexpectedCallException::MakeWhat(_zenMockedFunctionSignature, 1, 2, 3, 4, 5, 6, 7));
   }
   ...
   TEST(ZenMockIt_ExpectedTrue_IncrementsNumberOfCalls_CallsZenMockThrowIfExceptionSet)
   {
      ...
      //
      _zenMocker->ZenMockIt(1, 2, 3, 4, 5, 6, 7);
      //
      ...
      DOES_NOT_THROW(_zenMocker->CalledOnceWith(1, 2, 3, 4, 5, 6, 7));
      SetAssertedTrueToNotFailDueToExpectedButNotAsserted();
   }

   RUN_TESTS(SevenArgumentZenMockerTests)
}
