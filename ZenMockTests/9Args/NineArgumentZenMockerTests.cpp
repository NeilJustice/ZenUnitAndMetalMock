...
namespace ZenMock
{
   TESTS(NineArgumentZenMockerTests)
   ...
   using ZenMockerType = NineArgumentZenMocker<int, int, int, int, int, int, int, int, int, ExceptionThrowerMock>;
   ...
   {
      IS_FALSE(_zenMocker->_wasExpected);
      THROWS_EXCEPTION(_zenMocker->ZenMockIt(1, 2, 3, 4, 5, 6, 7, 8, 9), UnexpectedCallException,
         UnexpectedCallException::MakeWhat(_zenMockedFunctionSignature, 1, 2, 3, 4, 5, 6, 7, 8, 9));
   }
   ...
   TEST(ZenMockIt_ExpectedTrue_IncrementsNumberOfCalls_CallsZenMockThrowIfExceptionSet)
   {
      ...
      //
      _zenMocker->ZenMockIt(1, 2, 3, 4, 5, 6, 7, 8, 9);
      //
      using CallType = NineArgumentFunctionCall<int, int, int, int, int, int, int, int, int>;
      ...
      DOES_NOT_THROW(_zenMocker->CalledOnceWith(1, 2, 3, 4, 5, 6, 7, 8, 9));
      SetAssertedTrueToNotFailDueToExpectedButNotAsserted();
   }

   RUN_TESTS(NineArgumentZenMockerTests)
}
