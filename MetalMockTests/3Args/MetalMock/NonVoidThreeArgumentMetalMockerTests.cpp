...
namespace MetalMock
{
   ...
   {
      IS_FALSE(_nonVoidThreeArgumentMetalMocker->p_wasExpected);
      IS_FALSE(_nonVoidThreeArgumentMetalMocker->_derivedNonVoidCallInsteadFunction);
      //
      ...
      IS_TRUE(_nonVoidThreeArgumentMetalMocker->p_wasExpected);
   }
   ...
   TEST(MetalMockItAndReturnValue_CallInsteadPreviouslyCalled_CallsBaseMetalMockIt_ReturnsResultOfCallingCallInsteadFunctionOnce)
   {
      ...
      //
      const vector<tuple<int, int, int>> expectedCallInsteadFunctionArguments =
      {
         { arg1, arg2, arg3 }
      };
