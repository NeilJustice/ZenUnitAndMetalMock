...
namespace MetalMock
{
   TESTS(EightArgumentMetalMockerTests)
   ...
   using MetalMockerType = EightArgumentMetalMocker<int, int, int, int, int, int, int, int, MetalMockExceptionThrowerMock>;
   ...
   TEST(MetalMockIt_ExpectedTrue_IncrementsNumberOfCalls_CallsMetalMockThrowIfExceptionSet)
   {
      ...
      //
      _metalMocker->MetalMockIt(1, 2, 3, 4, 5, 6, 7, 8);
      //
      ...
      DOES_NOT_THROW(_metalMocker->CalledOnceWith(1, 2, 3, 4, 5, 6, 7, 8));
      SetAssertedTrueToNotFailDueToExpectedButNotAsserted();
   }

   RUN_TESTS(EightArgumentMetalMockerTests)
}
