...
namespace MetalMock
{
   TESTS(NineArgumentMetalMockerTests)
   ...
   {
      IS_FALSE(_metalMocker->_wasExpected);
      THROWS_EXCEPTION(_metalMocker->MetalMockIt(1, 2, 3, 4, 5, 6, 7, 8, 9), UnexpectedCallException,
         UnexpectedCallException::MakeExceptionMessage(_metalMockedFunctionSignature, 1, 2, 3, 4, 5, 6, 7, 8, 9));
   }
   ...
   TEST(MetalMockIt_ExpectedTrue_IncrementsNumberOfCalls_CallsMetalMockThrowIfExceptionSet)
   {
      ...
      //
      _metalMocker->MetalMockIt(1, 2, 3, 4, 5, 6, 7, 8, 9);
      //
      ...
      DOES_NOT_THROW(_metalMocker->CalledOnceWith(1, 2, 3, 4, 5, 6, 7, 8, 9));
      SetAssertedTrueToNotFailDueToExpectedButNotAsserted();
   }

   RUN_TESTS(NineArgumentMetalMockerTests)
}
